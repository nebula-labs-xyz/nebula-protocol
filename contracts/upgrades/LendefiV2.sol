 ...
 *  ¬─     ¬─   ¬─¬─  ¬─¬─'  ¬─¬─     ¬─'   ¬─¬─   '¬─    '─¬   ¬─      ¬─'           ¬─'
 *
 * @title Lendefi Protocol V2
 * @notice An efficient monolithic lending protocol
 * @author Lendefi Labs LLC
 * @custom:security-contact security@Lendefi-labs.xyz
 */
    ...
    YodaMath
{
    ...
    /**
     * @dev Supply USDC liquidity to protocol, and receive Lendefi tokens.
    ...
    /**
     * @dev Exchange Lendefi tokens back to USDC, receive yield.
     * @param amount to be exchanged in Lendefi yield token (6 decimals)
     */
    ...
    function decimals() public view virtual override(ERC20Upgradeable, IPROTOCOL) returns (uint8) {
        return IERC20Metadata(address(usdcInstance)).decimals();
    }

    /// @inheritdoc ERC20Upgradeable
    function balanceOf(address src) public view virtual override(ERC20Upgradeable, IPROTOCOL) returns (uint256) {
        return super.balanceOf(src);
    }

    /// @inheritdoc ERC20Upgradeable
    function totalSupply() public view virtual override(ERC20Upgradeable, IPROTOCOL) returns (uint256) {
